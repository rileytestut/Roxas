...
#import "RSTWrappedError.h"

NSErrorUserInfoKey const RSTLocalizedTitleErrorKey = @"RSTLocalizedTitle";
NSErrorUserInfoKey const RSTSourceFileErrorKey = @"RSTSourceFile";
NSErrorUserInfoKey const RSTSourceLineErrorKey = @"RSTSourceLine";

@implementation RSTWrappedError
...
- (instancetype)initWithError:(NSError *)error userInfo:(NSDictionary<NSString *,id> *)userInfo
{
    ...
    if (self)
    {
        if ([error isKindOfClass:[RSTWrappedError class]])
        {
            _wrappedError = [(RSTWrappedError *)error wrappedError];
        }
        else
        {
